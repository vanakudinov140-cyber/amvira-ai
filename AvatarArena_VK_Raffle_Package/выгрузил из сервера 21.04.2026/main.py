...
DEFAULT_STAGE_WINNERS_COUNT = int(os.getenv("DEFAULT_STAGE_WINNERS_COUNT", "3"))
...
BITRIX_WEBHOOK_TIMEOUT_SEC = float(os.getenv("BITRIX_WEBHOOK_TIMEOUT_SEC", "3"))
BITRIX_WEBHOOK_ENABLED = os.getenv("BITRIX_WEBHOOK_ENABLED", "1").strip() in {"1", "true", "yes", "on"}
...
def send_bitrix_webhook(event_type: str, payload: dict) -> None:
    if not BITRIX_WEBHOOK_ENABLED or not BITRIX_WEBHOOK_URL:
        return
    body = {
    ...
    try:
        with urllib_request.urlopen(req, timeout=BITRIX_WEBHOOK_TIMEOUT_SEC):
            pass
    except (urllib_error.URLError, TimeoutError, ValueError) as exc:
        print(f"[WARN] Bitrix webhook failed ({event_type}): {exc}")
...
def keyboard_main(include_admin: bool = False) -> str:
    buttons = [
        [
            {"action": {"type": "text", "label": "Проверить QR", "payload": {"cmd": "activate_pending"}}, "color": "positive"},
        ],
        [
...
def next_planned_stage_no(conn: sqlite3.Connection) -> int | None:
    row = conn.execute(
        "SELECT stage_no FROM stages WHERE status='planned' ORDER BY stage_no LIMIT 1"
    ).fetchone()
    return int(row["stage_no"]) if row else None


def keyboard_admin_panel(conn: sqlite3.Connection) -> str:
    active = active_stage(conn)
    next_stage_no = next_planned_stage_no(conn)
    if active:
        primary_btn = {
            "action": {
                "type": "text",
                "label": f"🎡 Запустить колесо фортуны (этап {int(active['stage_no'])})",
                "payload": {"cmd": "admin_draw_active_stage"},
            },
            "color": "positive",
        }
    elif next_stage_no:
        primary_btn = {
            "action": {
                "type": "text",
                "label": f"▶ Запустить {next_stage_no} этап",
                "payload": {"cmd": "admin_start_next_stage"},
            },
            "color": "positive",
        }
    else:
        primary_btn = {
            "action": {"type": "text", "label": "✅ Все этапы завершены", "payload": {"cmd": "admin_summary"}},
            "color": "secondary",
        }
    ...
    return json.dumps(
        {
            ...
            "buttons": [
                ...
                [
                    {"action": {"type": "text", "label": "Статистика", "payload": {"cmd": "admin_summary"}}, "color": "primary"},
                    {"action": {"type": "text", "label": "Экспорт", "payload": {"cmd": "admin_export"}}, "color": "secondary"},
                ],
                [
...
def keyboard_admin_reset_confirm() -> str:
    return json.dumps(
        {
            "one_time": False,
            "buttons": [
                [
                    {"action": {"type": "text", "label": "✅ Подтвердить", "payload": {"cmd": "admin_reset_cycle_confirm"}}, "color": "negative"},
                ],
                [
                    {"action": {"type": "text", "label": "↩️ Отмена", "payload": {"cmd": "admin_open"}}, "color": "secondary"},
                ],
            ],
        },
        ensure_ascii=False,
...
def format_stage(row: sqlite3.Row) -> str:
    starts = pretty_dt(row["starts_at"] or "")
    ends = pretty_dt(row["ends_at"] or "")
    ...
    status_text = status_map.get(status_raw, row["status"])
    return (
        f"Этап {row['stage_no']}\n"
...
def create_excel_report(conn: sqlite3.Connection, output_path: Path) -> None:
    ...
    desired_order = ["Participants", "Tokens", "Summary", "Draws", "Winners"]
    order_index = {name: idx for idx, name in enumerate(desired_order)}
    wb._sheets.sort(key=lambda ws: order_index.get(ws.title, len(desired_order)))
    wb.save(output_path)
...
def activate_token(conn: sqlite3.Connection, token: str, user_id: int) -> tuple[bool, str]:
    ...
    return (
        True,
        "🎉  Отлично! Ваш QR-код успешно активирован!\n"
        "Чем больше активированных кодов — тем ближе победа и главный приз 🏆\n"
        "Продолжайте собирать шансы — удача любит настойчивых 🍀",
    )
...
def run_draw_for_stage(conn: sqlite3.Connection, stage_no: int, winners_count: int) -> tuple[bool, str, set[int], set[int]]:
    ...
    winners_count = min(winners_count, len(candidates))
...
def start_stage_with_notifications(conn: sqlite3.Connection, vk, stage_no: int) -> str:
    ...
    if participants and result == "Этап активирован.":
        if stage_no == 2:
            stage_start_text = (
                "🏆 Внимание! Стартует 2 этап большого розыгрыша призов от «Avatar Arena»! 🎉\n\n"
                "Первый этап уже позади, и борьба за подарки становится жарче 🔥\n\n"
                "Если вы уже участвуете — увеличивайте шансы на победу.\n"
                "Если только присоединяетесь — сейчас идеальный момент войти в игру 🎟\n\n"
                "📲 Активируйте QR-коды, собирайте больше шансов и приближайтесь к заветному призу 🎁🍀\n"
                "Чем больше активированных купонов — тем выше вероятность, что удача выберет именно вас!\n\n"
                "Не упустите шанс стать победителем 🏆"
            )
        elif stage_no == 3:
            stage_start_text = (
                "🏆 Внимание! Стартует 3 этап большого розыгрыша призов в «Avatar Arena»! 🎉\n\n"
                "Второй этап уже позади, и борьба за подарки становится решающей 🔥\n\n"
                "Если вы уже участвуете — увеличивайте шансы на победу.\n"
                "Если только присоединяетесь — сейчас последний момент войти в игру 🎟\n\n"
                "📲 Активируйте QR-коды, собирайте больше шансов и приближайтесь к главному призу 🎁🍀\n"
                "Чем больше активированных купонов — тем выше вероятность, что удача выберет именно вас!\n"
                "Не упустите шанс стать победителем 🏆"
            )
        else:
...
def run_stage_draw_with_notifications(conn: sqlite3.Connection, vk, stage_no: int, winners_count: int) -> tuple[bool, str]:
    ...
    winner_text = (
        "🏆 ВЫ ПОБЕДИЛИ!!! 🎉\n"
        "Ваш купон стал выигрышным — поздравляем!\n"
        "Сегодня удача выбрала именно вас 🍀\n"
        "Совсем скоро с вами свяжется администратор для вручения приза 🎁\n"
        "Спасибо за участие и яркую игру вместе с Avatar Arena 💙"
    )
    loser_text = (
        f"⏳ {stage_no} этап завершён.\n"
        "До победы не хватило буквально шага.\n"
        "Ваши активные коды уже в следующем этапе — ничего не сгорает.\n"
        "Добавляйте новые чеки, увеличивайте количество шансов — "
        "следующим победителем можете быть именно вы 🍀"
    )
...
def reset_raffle_cycle(conn: sqlite3.Connection) -> str:
    ...
    return (
        "Новый цикл розыгрыша подготовлен: этапы сброшены, "
        "QR-активации и отметки победителей обнулены."
    )
                ...
                with db() as conn:
                    ...
                    send_bitrix_webhook(
                        "incoming_message",
                        {
                            ...
                            "message_text": text[:1500],
                            "payload_cmd": cmd or "",
                            "token_hints_count": len(token_hints),
                        },
                    )
                    ...
                    if is_admin and not payload.get("cmd"):
                        text_to_cmd = {
                            ...
                            "запустить этап": "admin_start_next_stage",
                            "запустить 1 этап": "admin_start_next_stage",
                            "запустить 2 этап": "admin_start_next_stage",
                            "запустить 3 этап": "admin_start_next_stage",
                            "следующий этап": "admin_next_stage",
                            "запустить колесо фортуны": "admin_draw_active_stage",
                            "колесо фортуны": "admin_draw_active_stage",
                            "новый цикл розыгрыша": "admin_reset_cycle",
                            "экспорт": "admin_export",
                            "экспорт excel": "admin_export",
                            "участники csv": "admin_export",
                    ...
                    if is_admin and cmd in legacy_disabled_cmds:
                        send(
                            ...
                            "Используйте актуальные кнопки этапов в админ-панели.",
                            keyboard=keyboard_admin_panel(conn),
                        )
                        continue
                    ...
                    if is_admin and cmd == "admin_next_stage":
                        # Legacy command: choose action by current stage state.
                        cmd = "admin_draw_active_stage" if active_stage(conn) else "admin_start_next_stage"
                    if is_admin and cmd == "admin_start_next_stage":
                        if active_stage(conn):
                            send(
                                vk,
                                user_id,
                                "Сначала завершите текущий активный этап через кнопку «🎡 Запустить колесо фортуны».",
                                keyboard=keyboard_admin_panel(conn),
                            )
                            continue
                        stage_no = next_planned_stage_no(conn)
                        if not stage_no:
                            send(vk, user_id, "Нет этапов для запуска: все этапы уже завершены.", keyboard=keyboard_admin_panel(conn))
                            continue
                        now = dt.datetime.now(dt.timezone.utc)
                        starts_at = now
                        ends_at = now + dt.timedelta(days=30)
                        conn.execute(
                            "UPDATE stages SET starts_at=?, ends_at=?, status='planned' WHERE stage_no=?",
                            (starts_at.isoformat(), ends_at.isoformat(), stage_no),
                        )
                        start_result = start_stage_with_notifications(conn, vk, stage_no)
                        send(
                            vk,
                            user_id,
                            f"✅ Этап {stage_no} успешно запущен.\n\n"
                            f"🗓 Период этапа:\n"
                            f"Старт: {pretty_dt(starts_at.isoformat())}\n"
                            f"Завершение: {pretty_dt(ends_at.isoformat())}\n\n"
                            f"{start_result}",
                            keyboard=keyboard_admin_panel(conn),
                        )
                        continue
                    if is_admin and cmd == "admin_draw_active_stage":
                        winners_count = DEFAULT_STAGE_WINNERS_COUNT if DEFAULT_STAGE_WINNERS_COUNT > 0 else 1
                        winners_count = max(1, min(3, winners_count))
                        active = active_stage(conn)
                        if not active:
                            send(
                                vk,
                                user_id,
                                "Сейчас нет активного этапа. Сначала запустите следующий этап.",
                                keyboard=keyboard_admin_panel(conn),
                            )
                            continue
                        stage_no = int(active["stage_no"])
                        draw_ok, draw_result = run_stage_draw_with_notifications(conn, vk, stage_no, winners_count)
                        if not draw_ok:
                            admin_text = f"⚠️ {draw_result}"
                        else:
                            next_stage_no = next_planned_stage_no(conn)
                            if next_stage_no:
                                admin_text = (
                                    f"🎉 Розыгрыш этапа {stage_no} успешно завершен.\n\n"
                                    f"{draw_result}\n"
                                    f"(Лимит победителей на этап: до {winners_count})\n\n"
                                    f"➡️ Следующий шаг: нажмите «▶ Запустить {next_stage_no} этап»."
                                )
                            else:
                                admin_text = (
                                    f"🎉 Розыгрыш этапа {stage_no} успешно завершен.\n\n"
                    ...
                    if is_admin and cmd == "admin_summary":
                        ...
                        send(vk, user_id, f"{result or 'Ошибка получения статистики.'}\n\n{stages_overview(conn)}", keyboard=keyboard_admin_panel(conn))
                        continue
                    if is_admin and cmd == "admin_reset_cycle":
                        send(
                            vk,
                            user_id,
                            "⚠️ Вы точно уверены, что хотите прекратить текущие розыгрыши и начать новый цикл?\n\n"
                            "Это действие сбросит текущие этапы.\n"
                            "Все QR-активации и отметки победителей будут обнулены.",
                            keyboard=keyboard_admin_reset_confirm(),
                        )
                        continue
                    if is_admin and cmd == "admin_reset_cycle_confirm":
                        result = reset_raffle_cycle(conn)
                        send(
                            vk,
                            user_id,
                            f"♻️ {result}\n\nНажмите кнопку запуска этапа, чтобы стартовать новый цикл.",
                            keyboard=keyboard_admin_panel(conn),
                        )
                        continue
                    if is_admin and cmd == "admin_export":
                        try:
                            ...
                            send_document(
                                ...
                                message="Готово: полный экспорт (включая участников).",
                            )
                            try:
                            ...
                            send(vk, user_id, f"Не удалось сформировать/отправить Excel: {exc}", keyboard=keyboard_admin_panel(conn))
                        continue
                    ...
                    if user_state:
                        ...
                        if state == "await_phone":
                            if cmd == "start_qr_flow" or low in {"start", "/start", "начать", "привет", "старт"}:
                                clear_user_state(conn, user_id)
                                if consented:
                                    send(
                                        vk,
                                        user_id,
                                        "🔥 Добро пожаловать в Avatar Arena — мир ярких эмоций, захватывающего драйва и настоящих побед! 🏆\n\n"
                                        "Здесь каждый QR-код — это ваш шанс сделать шаг навстречу ценным призам, крутым подаркам и незабываемым впечатлениям 🎁\n\n"
                                        "Получить QR-код для участия в розыгрыше можно у администратора в любом из наших филиалов — просто обратитесь к нему перед игрой 👌\n\n"
                                        "Готовы испытать удачу и стать частью игры? Тогда начинаем 👇\n\n"
                                        "Если хотите задать вопрос, нажмите на кнопку \"Связаться с администратором\".",
                    ...
                    if not consented:
                        ...
                        if is_consent_signal(cmd, low):
                            ...
                            send(
                                vk,
                                user_id,
                                "✅ Отлично! Согласие успешно получено.\n"
                                "Вы уже на шаг ближе к участию в розыгрыше 🎁\n"
                                "Остался последний шаг — отправьте номер телефона в формате:\n"
                                "+79991234567 📱\n"
                                "После этого ваше участие будет полностью подтверждено.",
                                keyboard=keyboard_phone_entry(),
                            )
                        elif cmd == "start_qr_flow" or text.lower() in {"старт", "start", "/start"}:
                            send(
                                vk,
                                user_id,
                                "🔥 Добро пожаловать в Avatar Arena — мир ярких эмоций, захватывающего драйва и настоящих побед! 🏆\n\n"
                            ...
                            if token_hints:
                                send(
                                    ...
                                    "Нажмите «Старт», чтобы перейти в меню участия 👇",
                                    keyboard=keyboard_start_from_qr(),
                                )
                            else:
                                send(
                                    vk,
                                    user_id,
                                    "👋 Добро пожаловать!\n\n"
                                    "Чтобы участвовать в розыгрыше, нажмите кнопку ниже и пройдите регистрацию.\n"
                                    "QR-код можно активировать после входа по ссылке с купона 🎟️\n\n"
                                    "Если хотите задать вопрос, нажмите на кнопку \"Связаться с администратором\".",
                                    keyboard=keyboard_qr_entry(),
                                )
                        continue

                    # For explicit "start" actions, always show greeting first.
                    if token_hints and (cmd == "start_qr_flow" or low in {"старт", "start", "/start", "начать", "привет"}):
                        save_pending_token(conn, user_id, token_hints[0])
                        send(
                            vk,
                            user_id,
                    ...
                    if low in {"правила", "rules"}:
                        send(
                            vk,
                            user_id,
                            "🎟 1 активированный QR-код = 1 дополнительный шанс на выигрыш\n\n"
                            "🔒 Каждый QR-код уникален и может быть использован только один раз\n\n"
                            "⚠️ Если код уже был активирован другим участником, повторное использование становится недоступным\n\n"
                            "🏆 Чем больше QR-кодов вы активируете, тем выше ваши шансы оказаться среди победителей\n\n"
                            "🍀 Каждый новый код приближает вас к заветному призу — удача любит тех, кто идёт до конца!",
                            keyboard=keyboard_main(include_admin=is_admin),
                        )
                        continue
                    if cmd == "activate_pending":
                        ...
                        if not pending:
                            send(
                                vk,
                                user_id,
                                "🎟️ Сейчас у вас нет QR-кода, ожидающего активации.\n\n"
                                "Чтобы получить новый шанс на победу 🏆, пожалуйста, перейдите в бот повторно через QR-код с вашего купона.\n\n"
                                "🍀 После перехода система автоматически распознает код и подготовит его к активации.",
                                keyboard=keyboard_main(include_admin=is_admin),
                            )
                            continue
                        if not participant_registered(conn, user_id):
                    ...
                    if low in {"мои шансы", "шансы"}:
                        ...
                        send(
                            vk,
                            user_id,
                            f"🎯  Ваш личный баланс удачи прямо сейчас:\n🎟️ {chances} шанс(ов)\n\n"
                            "Каждый активированный QR-код повышает вероятность победы.\n"
                            "Чем больше шансов — тем ближе ваш приз 🎁\n"
                            "Не останавливайтесь, удача уже рядом 🏆",
                            keyboard=keyboard_main(include_admin=is_admin),
                        )
                        continue
                    ...
                    if is_admin and low in {"админ-панель", "панель", "admin"}:
                        send(vk, user_id, "Панель администратора.\nВыберите действие кнопками ниже.", keyboard=keyboard_admin_panel(conn))
                        continue
                    if not text:
                    ...
                    send(
                        ...
                        "Но ничего страшного — мы быстро разберёмся!\n"
                        "Просто выберите нужное действие ниже, и я помогу вам продолжить путь к победе 👇",
                        keyboard=keyboard_main(include_admin=is_admin),
                    )
        except Exception as exc:
