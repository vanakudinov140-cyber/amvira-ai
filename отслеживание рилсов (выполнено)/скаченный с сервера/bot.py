...
def moscow_today() -> date:
    """Календарный день по Москве — как у расписания напоминаний и ожидания учениц."""
...
async def safe_send(chat_id, text, **kwargs):
    while True:
        try:
            ...
            logging.warning("Сетевая ошибка при отправке пользователю %s: %s", chat_id, e)
            await asyncio.sleep(3)
        except TelegramServerError as e:
            logging.warning("Ошибка Telegram при отправке пользователю %s: %s", chat_id, e)
            await asyncio.sleep(3)
        except TelegramAPIError as e:
            # Forbidden, BadRequest, Unauthorized и др. — не крутим цикл бесконечно.
            logging.warning("Не удалось отправить сообщение пользователю %s: %s", chat_id, e)
            return None
...
async def safe_send_photo(chat_id, photo_path, caption, **kwargs):
    while True:
        try:
            ...
            logging.warning("Сетевая ошибка при отправке фото пользователю %s: %s", chat_id, e)
            await asyncio.sleep(3)
        except TelegramServerError as e:
            logging.warning("Ошибка Telegram при отправке фото пользователю %s: %s", chat_id, e)
            await asyncio.sleep(3)
        except TelegramAPIError as e:
            logging.warning("Не удалось отправить фото пользователю %s: %s", chat_id, e)
            return None
...
async def reminder_16():
    ...
    logging.info(
        "reminder_16: старт, пользователей в БД=%s, день=%s",
        len(users),
        moscow_today_str(),
    )
    ...
    for (uid,) in users:
        try:
            ...
            if count < 2:
                ...
                if image_exists(REMINDER_16_IMAGE):
            ...
            logging.exception("Ошибка в reminder_16 для user_id=%s: %s", uid, e)

    logging.info("reminder_16: завершено")
...
async def reminder_20():
    ...
    logging.info(
        "reminder_20: старт, пользователей в БД=%s, день=%s",
        len(users),
        moscow_today_str(),
    )
    ...
    for (uid,) in users:
        try:
            ...
            if count < 2:
                remaining = 2 - count
                text = f"время выложить контент!!🌟\nНа сегодня осталось: {remaining}/2"
                if image_exists(REMINDER_20_IMAGE):
                    await safe_send_photo(
                        uid,
                        REMINDER_20_IMAGE,
                        text,
            ...
            logging.exception("Ошибка в reminder_20 для user_id=%s: %s", uid, e)

    logging.info("reminder_20: завершено")
...
async def main():
    ...
    # Явный часовой пояс + запас на задержку цикла событий (иначе cron мог «промахнуться»).
