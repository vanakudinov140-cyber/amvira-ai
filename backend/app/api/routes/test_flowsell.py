...
<head>
  ...
  <title>FlowSell demo send</title>
  <style>
    :root {
      color-scheme: light;
      --bg: #f7f8fb;
...
<body>
  <main>
    <section class="card">
      <h1>FlowSell demo send</h1>
      <p>Internal tool для ручной проверки single WhatsApp sends через существующий controlled FlowSell pipeline.</p>
      <div class="notice">
        Emergency rollback: установите <strong>FLOWSELL_DRY_RUN=true</strong> и <strong>TEST_MODE=true</strong>.
        Не используйте эту страницу для batch, campaigns, scheduler или automation rollout.
      </div>
      ...
      <section id="result" class="result">
        <div class="summary">
          <div class="metric"><span>sent</span><strong id="sentValue">-</strong></div>
          <div class="metric"><span>dry_run</span><strong id="dryRunValue">-</strong></div>
          <div class="metric"><span>message_id</span><strong id="messageIdValue">-</strong></div>
  ...
  <script>
    ...
    const validateForm = () => {
      ...
      if (phoneDigits.length < 10) {
        return "Укажите phone number минимум из 10 цифр.";
      }
      if (!getValue("booking_link").startsWith("http")) {
        return "Укажите реальную ссылку на запись, начинающуюся с http или https.";
      }
      return "";
    };

    const renderResult = (data) => {
      document.getElementById("sentValue").textContent = String(data.sent);
      document.getElementById("dryRunValue").textContent = String(data.dry_run);
      ...
      resultJson.textContent = JSON.stringify({
        ...
        provider_response_preview: data.provider_response_preview,
    ...
    form.addEventListener("submit", async (event) => {
      ...
      setStatus("Отправка через controlled pipeline...", "");
      try {
        ...
        renderResult(data);
        if (!response.ok) {
          setStatus(`Ошибка HTTP ${response.status}`, "error");
        } else if (data.sent) {
          setStatus("Отправлено успешно.", "success");
        } else if (data.dry_run) {
          setStatus("Dry-run: provider call не выполнялся.", "success");
        } else {
          setStatus("Сообщение не отправлено. Проверьте warnings/errors.", "error");
        }
      } catch (error) {
        setStatus(`Ошибка запроса: ${error}`, "error");
      } finally {
